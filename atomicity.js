$(document).ready(function() {
   ...
   // this function is a total mess. may god help you.
   ...
   $('.filter-bar').on('keydown change', '.input', function() {
      var $this = $(this);
		var $context = $this.parents('.context');
		var $filteredCategories = $.grep($this.val().split(/\s*,\s*/), function(category) { return category !== '' });
		$.each($context.find('.input-container .entry.section-group'), function(_, entry) {
			...
			if ($filteredCategories.length == 0) {
				...
				for (var i = 0; i < $filteredCategories.length; i++) {
					if ($categories !== null && $categories !== undefined && $categories.indexOf($filteredCategories[i]) !== -1) {
						$foundCategory = true;
						$entry.removeClass('hidden');
						break;
					}
   ...
   $('.output-container .controls button.save').on('click', function() {
      var $context = $(this).parents('.context');
      if ($context.find('.input-container .section.invalid-empty, .input-container .section.invalid-no-category, .input-container .section.invalid-timestamp').length > 0) {
   ...
   $('.output-container .controls button.download').on('click', function() {
      var $context = $(this).parents('.context');
      if ($context.find('.input-container .section.invalid-empty, .input-container .section.invalid-no-category').length > 0) {
         alert("Can't save while there are errors. correct fields marked in red.");
         return;
      }
      ...
      $.each($context.find('.input-container .section-group'), function(_, sectionGroup) {
         ...
         $.each($(this).find('.section'), function(_, section) {
            ...
            if ($section.hasClass('timestamp')) {
               ...
               fieldValue = currentTime !== null && currentTime.isValid() ? currentTime.utc().format('YYYY-MM-DDTHH:mm:ss\\Z') : '';
            }
   ...
   function checkForEmptiness($input) {
	...
	function checkForCicularInheritance($title, $inheritedCategories) {
		while ($inheritedCategories.length != 0) {
			...
			$inheritedCategories = $inheritedCategories.concat($.grep(($currEntry.find('.section[data-name="category"][data-attr="term"] .input').val() || '').split(/\s*,\s*/), function(caregory) { return caregory !== '' }));
		}
		return true;
	}
